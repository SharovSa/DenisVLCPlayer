...
class GlobalPlayer:
    def __init__(self):
        ...
        self.savefrom_btn_image = PhotoImage(file='logo/savefrom.png')
        ...
        self.savefrom_btn = Button(self.control_frame, image=self.savefrom_btn_image, borderwidth=0, command=self.add_music_from_yt)
    ...
    def add_music_from_yt(self):
        url = askstring("Введите ссылку","")
        Converter.convert_music_from_yt(url)
        self.load_music()
        player_owner.get_manager().get_all_songs().check_memory()
    def load_music(self):
        """
        ...
        # Очистить список songs перед загрузкой новых треков
        songs.clear()
        for song in os.listdir(root.directory):
            name, ext = os.path.splitext(song)
            if ext == '.mp3':
                songs.append(song)
        # Очистить существующий список треков в виджете self.songlist
...
if __name__ == "__main__":
    ...
    global_player.savefrom_btn.grid(row=0, column=9, padx=7, pady=10)
